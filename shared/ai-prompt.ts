...
// Find the first top-level balanced JSON object inside `s`. Returns the
// substring (including the outer braces) or null if no balanced object is
// found. Tolerates strings containing braces and escape sequences. We only
// look at the FIRST balanced object so prose preambles and trailing model
// commentary are stripped cleanly.
...
// Parse the model's raw output into the canonical response shape. Tolerates:
//   - markdown code fences (```json … ```)
//   - leading or trailing prose
//   - missing optional fields (altTitles / hashtags / commentGuide / warnings)
//
// When `strict` is true (default), throws on irrecoverable input. The Vercel
// handler and Express dev server pass strict=false so they can fall through to
// the prose-synthesis branch downstream instead of returning a hard error.
export function parseModelOutput(
  ...
  // If the model produced prose around the JSON object, isolate the first
  // balanced { … } block — `lastIndexOf("}")` from the earlier implementation
  // would attach trailing prose punctuation when the model emitted extra
