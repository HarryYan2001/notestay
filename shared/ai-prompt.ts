// Prompt construction + response parsing for the Zhipu (BigModel) AI
// generation route. Pure / dependency-free so it can be reused from:
//   - api/generate-note.ts (Vercel serverless function)
//   - server/routes.ts (Express dev route)
//   - script/smoke-ai-*.ts (offline tests, no network required)
//
// Design goals:
//   1. Model only generates Chinese Xiaohongshu hotel-review TEXT (title,
//      body, hashtags, comment guide). All cover / page visuals stay
//      deterministic on the client.
//   2. Strong guardrails: model must NOT invent prices, services, locations,
//      facilities, breakfast items, or any fact the user did not supply.
//   3. OCR-learned reference style is passed as STYLE METHOD only — never
//      as content to copy. Strength materially affects 正文 rhythm.
//   4. Output is strict JSON so the client can parse without heuristics.
