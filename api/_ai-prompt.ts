...
// the AI note generation feature.
//
// IMPORTANT: This file lives inside `api/` (with a leading underscore so
// Vercel does NOT treat it as a serverless function — see
// https://github.com/vercel/vercel/discussions/4983). It is the canonical
// home of the runtime so that `api/generate-note.ts` can pull it in via a
// plain STATIC sibling import (`./_ai-prompt`), which Vercel's file tracer
// resolves reliably.
//
// Previous attempts to keep the canonical copy in `shared/ai-prompt.ts`
// failed in production:
//   1. A top-level static import of `../shared/ai-prompt` caused a cold-start
//      crash on Vercel (FUNCTION_INVOCATION_FAILED) — the tracer / bundler
//      did not reliably resolve sibling TS files from the function module.
//   2. Switching to a dynamic `await import("../shared/ai-prompt")` avoided
//      the cold-start crash but then failed at request time with
//      "Cannot find module '/var/task/shared/ai-prompt'" because the file
//      had not been included in the bundle at all (NFT cannot statically
//      analyze dynamic-import string paths).
//
// Keeping the runtime inside `api/` (and re-exporting from
// `shared/ai-prompt.ts` for server / client / test consumers) sidesteps
// both failures: there is exactly one static, sibling-relative import and
// Vercel always bundles every non-_ file in `api/`.
//
...
export interface AiImageSummary {
  // We deliberately DO NOT send raw image bytes. The model is told only the
  // count and high-level categories the user tagged so it can mention them
  // when relevant — never to invent visual details.
  totalCount: number;
...
export interface AiScreenshotStyleRef {
  hasText: boolean;
  tone: string;            // dramatic / healing / premium / playful / informative
  cues: string[];          // raw cue keys, e.g. heavy_emoji, cta_collect …
  cueLabels: string[];     // human-readable labels for the same cues
  detectedEmoji: string[]; // up to ~8 emoji glyphs spotted in OCR text
  punctIntensity: number;  // 0..1
...
export interface AiGenerateRequest {
  ...
  framework: AiFrameworkField[];      // only populated in framework mode
  freeText: string;                   // only populated in freeform mode
  hotel: AiHotelInfo;
  images: AiImageSummary;
  styleKey: string;                   // visual style key (xhs_burst etc.) — for tone hint only
  styleName: string;                  // human-readable style label
  styleTone: string;                  // styleDef vibe string
...
export interface AiGenerateResponse {
  title: string;
  altTitles: string[];     // 2-3 alternative titles
  body: string;            // markdown-ish, sections separated by \n\n
  hashtags: string[];      // exactly 3 by default, "#xxxx" form
  commentGuide: string[];  // 3-5 comment-bait seeds
  warnings: string[];      // safety / missing-info notes
}
...
// Build the system prompt. Constant across requests so it stays cacheable.
...
// Build the user prompt. Includes user content + style guidance + reference
// profile. Reference facts (style profile) are clearly fenced as METHOD-only.
...
// Parse the raw model output. Tolerates ```json fences but expects a JSON
// object with the documented shape. Throws on irrecoverable garbage.
export function parseModelOutput(raw: string): AiGenerateResponse {
  ...
  // Some models may return prose followed by JSON. Try to locate the
  // outer-most JSON object via brace balancing.
  if (!body.startsWith("{")) {
